...
| **Interaction design** | How the interface *behaves* over time — streaming, scrolling, loading, cancel, approve, undo | "What happens while the user is mid-task, especially when the system is busy?" |
...
| **Affordance** | A cue that something *can* be used a certain way (a button looks pressable) |
...
6. **One primary action** — Is there a clear main next step, or are equal-looking buttons competing?
7. **Wayfinding** — Do you always know which studio, agent, session, and job you are in?
8. **Interruptibility** — Can a long run be paused, cancelled, or approved without losing place?
9. **Empty and error states** — Does "nothing here" or "this failed" explain what to do next?
10. **Duplicate levers** — Did the same job get two entry points that behave differently?
11. **Signal vs noise** — Are badges only for things that need action, not every ambient state?
12. **Mobile / narrow pane** — Does the layout still work when the window is small (two-line rows, no horizontal table overflow)?
...
| **Visual states** | Hover, focus, active, disabled, loading — how it *looks* in each state (behavior of those states is interaction) |
