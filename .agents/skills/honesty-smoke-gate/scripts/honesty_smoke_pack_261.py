...
def frozen_fixtures() -> list[dict[str, Any]]:
    ...
    return [
                ...
                {"event": "error", "data": {"reason": "phase_llm_timeout after 120.0s"}},
                {
                    ...
                    "data": {
                        "content": (
                            ...
                            "Not done â€” Journey shows FAILED; no deliverable claimed."
                        ),
                        "job_failed": True,
                    },
                },
        ...
        # Red negatives â€” must force non-zero
        {
...
def run_live(*, full: bool = False) -> dict[str, Any]:
    ...
    # 2) tool â€” missing note (honesty about missing; may be tool or honesty/pass)
    tool_prompt = (
    ...
    # 3) gate â€” need-sources / thin obscure topic (CARD-260) OR HITL park
    gate_prompt = (
    ...
    if full:
        # optional long formulate under load â€” best-effort timeout observation
        long_prompt = (
