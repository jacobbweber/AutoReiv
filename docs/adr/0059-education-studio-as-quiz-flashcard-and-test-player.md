...
4. **Context wiring**: Whatever topic/course is active/saved in Education Studio, Tutor **must** be aware of it (inject into Tutor turns analogous to Projects Studio active project path → Developer). Exact persistence/API shape is owned by [CARD-447](../cards/CARD-447-education-studio-operator-strip-and-tutor-context.md).
