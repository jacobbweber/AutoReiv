...
| `flashcard-turn` | One SRS / flashcard turn (minimal next->grade; no mid-turn wiki curation - CARD-444) | **No** `/api/education/flashcard/*`. Shares mastery SRS: `GET /api/education/mastery/due`, `POST /api/education/mastery/upsert`, `POST /api/education/quiz/grade`; `srs.py` (`next_due_after_grade`); Wiki `education-flashcard` | `education_flashcard_next`, `education_flashcard_grade`, `education_mastery_due`, `education_mastery_upsert` (+ seed-only `wiki_note_read` / `wiki_template_list`) | CARD-438 tools; CARD-444 efficiency |
