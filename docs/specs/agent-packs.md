...
<pack-id>/
  ...
  # Live DBs are created under user-data packs/<id>/ at runtime — never commit *.db
  # <pack_id>_storage.db  # domain application data (CARD-148)
  # <pack_id>_memory.db   # cognitive brain (CARD-116)
```
...
- Do **not** include `*.db`, transcripts, person facts, secrets, `input_packet_json`, or Python tool implementations. Databases live only under the user data root. Tool ids under each skill are callables that already exist on the platform.
