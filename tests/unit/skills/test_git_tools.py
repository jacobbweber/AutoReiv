...
@pytest.fixture
def skill(repo: Path) -> GitTools:
    return GitTools(default_project_root=str(repo))


def test_status_diff_and_conventional_reject(skill: GitTools, repo: Path):
...
def test_commit_success_and_jail(skill: GitTools, repo: Path):
...
def test_status_skip_commit_when_not_a_repo(tmp_path: Path):
    skill = GitTools(default_project_root=str(tmp_path))
    status = skill.git_status()
