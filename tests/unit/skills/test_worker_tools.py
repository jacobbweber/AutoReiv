...
@pytest.fixture
def wiki_tools(tmp_path):
    wiki_dir = tmp_path / "wiki"
    wiki_dir.mkdir()
    return WikiTools(wiki_root=wiki_dir)


@pytest.fixture
def worker_tools(store, wiki_tools, temp_workspace):
    return BatchWorkerTools(
        state_store=store,
...
@pytest.mark.asyncio
async def test_batch_worker_scan_execution(worker_tools, store, temp_workspace):
    session = store.create_session(agent_id="assistant", title="Scan Session")

    result = await worker_tools.batch_worker_scan(
...
@pytest.mark.asyncio
async def test_batch_worker_scan_glob_pattern(worker_tools, store, temp_workspace):
...
@pytest.mark.asyncio
async def test_promote_artifact_to_wiki(worker_tools, store, wiki_tools):
    session = store.create_session(agent_id="assistant", title="Promotion Session")

    scan_res = await worker_tools.batch_worker_scan(
        session_id=session.id,
        paths="src/*.py",
    ...
    promo_res = worker_tools.promote_artifact_to_wiki(
        artifact_id=art_id,
...
def test_tool_registry_registration(worker_tools):
    reg = ScopedToolRegistry()
    worker_tools.register_tools(reg)
