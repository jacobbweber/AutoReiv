...
@pytest.fixture
def skill(tmp_path: Path) -> ProjectFileSkill:
    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "app.py").write_text("print(1)\n", encoding="utf-8")
    return ProjectFileSkill(default_project_root=str(tmp_path))


def test_list_read_write_under_root(skill: ProjectFileSkill, tmp_path: Path):
...
def test_jail_denies_dotdot_and_outside(skill: ProjectFileSkill):
