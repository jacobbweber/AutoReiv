...
def test_dry_run_and_mock_create(tmp_path: Path):
    cards = CardSkill(default_project_root=str(tmp_path))
    ...
    skill = GitHubIssueSkill(default_project_root=str(tmp_path), card_skill=cards)
    ...
    skill = GitHubIssueSkill(default_project_root=str(tmp_path), card_skill=cards, runner=runner)
...
def test_missing_gh_error(tmp_path: Path, monkeypatch):
    ...
    (tmp_path / ".github" / "cards").mkdir(parents=True)
    cards.write_card(content=CARD, filename="CARD-400-issue-sync.md")
    monkeypatch.setattr("src.application.skills.github_issue_skill.shutil.which", lambda _name: None)
    skill = GitHubIssueSkill(default_project_root=str(tmp_path), card_skill=cards)
