...
### Added
- CARD-337: Granular Telemetry Attribution & Performance Audit — Implemented discrete token and timing telemetry attribution and automated performance/cost auditing across AutoReiv:
  - Platform Direct Agent: Added `direct` as a built-in platform agent (`platform-packs/direct/pack.json`) with zero tools and zero skills for raw model benchmarking and pure, cost-effective chat.
  ...
  - Performance & Cost Audit Service & Tool: Implemented `AuditService` (`src/application/observability/audit_service.py`) and registered `audit_performance_and_cost` tool (`src/application/skills/audit_tools.py`) for the `autoreiv` platform agent, aggregating metrics across jobs, sessions, or time windows, computing Scaffold Ratio ("Harness Tax") and dollar costs, raising tool bloat warnings (>50%), and publishing structured reports directly to Wiki Studio under `01_Engineering/Performance/`.
  - Daily Performance & Cost Audit Routine: Seeded `daily-performance-audit` routine running at 2:00 AM daily to automatically audit the trailing 24 hours and export reports to Wiki Studio.
