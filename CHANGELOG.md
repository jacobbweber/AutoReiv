...
- CARD-207 In Review (`AutoReiv.Web`, `AutoReiv.UI`, `AutoReiv.Desktop` - CARD-207):
  ...
  - **Dynamic Sessions Window Stacking**: Removed hardcoded `z-index: 50 !important` from `#sidebar`, synchronizing its stacking position dynamically with the desktop window manager (`--dw-z`). Added prominent diagonal grip styling to bottom-right corner resize handles (`.desktop-win-resize-se`).
