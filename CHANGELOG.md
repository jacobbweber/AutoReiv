...
- CARD-357: Homelab Admin Deepening and Multi-Capability Factory Evolution — Evolved the Agent Training Factory into a multi-capability, non-destructive pack expansion engine and deepened `homelab-admin` with direct Hyper-V VM lifecycle and switch management:
  ...
  - Real Project Grounding (`src/application/agent_training_factory/phases/ground.py`, `src/application/skills/environment_inspection.py`): Plumbed active project directory into `GroundPhase`. Invoked `EnvironmentInspectionSkill` on real target directories to discover files, binaries, and prioritize project scripts (`.tf`, `.hcl`, `.yml`, `.yaml`, `.ps1`, `.py`, `.sh`), injecting `script_files` directly into downstream prompts.
