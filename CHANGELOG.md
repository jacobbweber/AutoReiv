...
- CARD-195 In Review (`AutoReiv.Web`, `AutoReiv.Frontend`, `AutoReiv.Orchestration` - CARD-195):
  ...
  - **Single Hub Agent Context Dropdown**: Integrated `<select id="factoryAgentSelect">` directly in the Factory Studio header, dynamically populated from `/api/agents` with `All Agents (Platform View)` and all loaded agents (`[REQ-FACT-040]`).
  - **Agent-Scoped Telemetry & Pre-Scoped Launcher**: Selecting an agent automatically filters historical and active runs, updates status filter counters and active run badges, and turns the primary launch action into `[ 🚀 Train <agent_name> ]` pre-scoped with target ID and starter objectives (`[REQ-FACT-041]`).
  - **Agent Studio Training Hub Consolidation**: Wired Agent Studio (`#view-forge`) `#forgeTrainAgentBtn` and `#forgeLabMonitorBtn` to transition directly to Factory Studio with the active agent selected in `#factoryAgentSelect`, consolidating all training into a single operational surface (`[REQ-FACT-042]`).
  ...
  - **Explicit Target Agent Selector & Live Pack Indicator**: Embedded `<select id="trainAgentTargetSelect">` and live inspection indicator `<div id="trainAgentLiveInfo">` inside `#trainAgentHandshakeModal`. When an existing agent is chosen, the modal displays its on-disk pack location (`packs/<agent_id>/`), existing skills count, and registered tools count, guaranteeing that training runs against the target agent and avoids duplicate tool definitions. When `+ Create Brand New Agent...` is chosen, the name authoring field is revealed (`[REQ-FACT-043]`).
  - **Automated Verification & Zero Quality Gaps**: Added comprehensive frontend unit tests in `tests/unit/frontend/factory_studio.test.js` and `tests/unit/frontend/train_agent_handshake.test.js` (271 frontend tests passing 100%), zero eslint errors, zero ruff errors, and full RTM validation for `[REQ-FACT-034]` through `[REQ-FACT-043]`.
