...
- CARD-165 In Review (`AutoReiv.Kernel`, `AutoReiv.Orchestration`, `AutoReiv.Web`, `AutoReiv.Agents`, `AutoReiv.Frontend` - CARD-165):
  ...
  - **Chat Studio Training Trigger**: Added `[⚡ Train in Lab]` message action button (`.train-lab-msg-btn`) to assistant chat bubbles to immediately queue capability gaps and launch the Lab handshake modal [REQ-FACT-028].
