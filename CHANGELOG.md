...
- **CARD-520 `tool_escalation`: one name for "this needs a tool", and Ask Developer in Observability (ADR-0060)**:
  ...
  - **Observability friction cards**: a tool escalation shows a **Needs a tool** badge with **Ask Developer** and **Dismiss** (no Apply). Ask Developer opens a real Developer chat that streams at once (the CARD-497 send path), then `POST /api/observability/friction/recommendations/{id}/escalate` marks the card **Asked Developer** and stores the Developer session; audits do not re-create it. Gap backlog, Teach and Observability share one helper, `askDeveloperWithDraft` in `tools_studio_authoring.js`.
  - **Apply tells the truth**: a tool escalation answers 409 "needs a tool change … Use Ask Developer", a tool no skill lists answers 409 "No skill lists `<tool>`, so there is nothing to patch", a missing skill file 404; the toast shows that message instead of "HTTP 500".
  - **Teach still sees a missing tool without the model**: when the distill model call fails, times out or returns nothing (a reasoning model can spend all 800 tokens thinking), guidance such as "AutoReiv has no weather tool; it needs a tool that ..." now gives a **Needs a tool** proposal (suggested name `get_weather`) instead of a template runbook. Other guidance still gets the runbook fallback (CARD-503 covers the timeout itself).
  - **Ask Developer names the agent that needs the tool**: Teach, gap and Observability drafts carry `target_agent_id`; the Developer prompt adds "Target agent: autoreiv. Register with grant_agent_ids ["autoreiv"] so autoreiv can call it." (only when set; unsafe ids are dropped). Before, only a prose line named the agent and `get_weather` was registered with no agent granted.
  - **Observability sections are all top level** (CARD-526 folded in): two missing `</details>` tags nested Journey Canvas, Standing Journey and Capability Catalog inside Live System Logs, and the friction section inside Capability Catalog.
  - Contracts: `tests/unit/core/test_card_520_tool_escalation.py`, `tests/unit/frontend/card_520_tool_escalation.test.js`, smoke TC-45 ([CARD-520], [CARD-526]).
- **CARD-497 The Agent Training Factory backend is gone; Skill Studio and Tools Studio have their own routes (ADR-0060)**: CARD-496 removed the Factory UI; this removes everything behind it.
  - **New routes, same payloads**: `GET /api/tools_studio/capabilities`, `POST /api/skill_studio/runbook`, `POST /api/skill_studio/save`, `GET /api/skill_studio/skills` and `GET /api/skill_studio/skills/{id}` (new `routers/skill_studio.py`). Skill Studio, silent runbook drafting, the skill picker, skill open and the Tools Studio catalog call them. The five old `/api/agent_training_factory/*` paths answer **308** (method, body and query kept) for one release; CARD-498 removes them.
  - **Removed (404)**: Factory jobs (`/jobs`, `/jobs/{id}`, `/step`, `/promote`, `DELETE`), `/gaps`, the phase instruction routes, and `POST /api/agents/{id}/gaps/{gap}/train`. Gaps still create, list and dismiss, and open Skill Studio or a Developer chat.
  - **Deleted code**: `application/agent_training_factory/` (orchestrator, phases, prompt registry, gap_link, SOP rubric), the Factory router, `factory_dispatch_tools.py` and the `launch_factory_training` chat tool, `capability_graph`, `jit_synthesizer`, `tool_synthesizer`, `verification_battery`, `hyperv_tool_builders`, `sandbox_runner`, the Factory packet repository and store mixin, and the Factory busy checker. The `factory_*` tables and their rows are kept for CARD-498.
  - **Kept and moved**: `inspect_agent_pack` is now in `AgentPackTools` (same name, output and grant); the runbook LLM helper is `application/skills/studio_llm.py`; gap statuses live in the gap repository.
  - **AutoReiv `agent-authoring` is now "Agent Capability Intake"**: it inspects the agent, then hands a tool or MCP request to the Developer with a brief, sends a skill to Skill Studio or `propose_skill`, and a new agent to build-agent-pack. `launch_factory_training` is on the retired-tools list, so pack refresh strips it from stored profiles, including edited ones.
    - Its description now says when to use it ("teach", "new capability", "learn to") and names its id for `skill_view(pack_id="agent-authoring")`, and the protocol spells out `handoff_to_agent(target_agent_id="developer", task_directive=<brief>)`. In the live retest before this change, AutoReiv never opened the skill and called `handoff_to_agent(agent_id=..., task=...)`, which failed; the reply then left the failure out. AutoReiv's system prompt gains one routing line: teach / new-capability requests open `skill_view(pack_id="agent-authoring")` (a retest with the skill text alone still called `activate_skill`, which only loads platform tool domains, and a positional `skill_view('agent-authoring')` hint made it guess the argument name).
  - **Ask Developer is a real send**: Ask Developer on a capability gap, Tools Studio **Talk to Developer** and Teach's **Ask Developer to build this tool** now start a Developer reply right away, as if the operator typed the request and pressed Enter. Before, `/api/tools_studio/authoring/talk` saved the request as a user message and the chat only showed it, so Developer did nothing until the operator typed again. The request is sent once: a double click, reopening the session or opening it on another device does not send it again, and if the chat is already busy the request goes into the message box with a notice instead. Tests: Vitest `card_497_developer_intent_send`, pytest 12e, OC-422, smoke TC-44 on desktop and phone (REQ-497-016).
  - **Startup**: gaps left in `training` by the Factory go back to `pending`. An unedited `build-agent-pack` seed in the data folder is refreshed to the new text (no Factory lines); an edited one is left alone and logged.
  - **API**: agents and pack exports no longer carry `allow_autonomous_training` / `max_training_retries` (sent values are ignored; old manifests still import). The `auto_train_progress` chat event is gone. Skill Studio save writes under the app's data folder instead of re-reading `AUTOREIV_DATA_DIR`.
  - Tests: 197 Factory-only tests (189 unit, 8 integration) were deleted with the code; 47 new pytest tests, 5 Vitest tests and smoke TC-43 (Skill Studio generate, save and open on desktop and phone) cover what stays.
- **CARD-511 A Developer-built tool runs once in the sandbox before it is registered (ADR-0060 D6)**: Before, a native tool with a syntax error, an import error or a crash registered fine and only failed when an agent called it, and a Developer-registered MCP server was saved even when it could not start. Now:
  - **Native tools** (`POST /api/tools/native` and Developer's `register_native_tool`): the code must parse, define a module-level `run`, pass the path-safety rule, import in the sandbox (10 s) and answer one sample call (20 s) with a JSON result. The sample input is Developer's new `sample_arguments`, or the minimum built from the tool's schema; the check never calls an LLM and gives the sandbox no credentials. `risk_level: high`, or `sample_call: "skip"` with a `skip_reason`, skips only the call.
  - **MCP servers** registered by Developer (`register_mcp_service`): the server must start, list at least one tool with a valid name and an object input schema (10 s), and answer one sample call (20 s). Settings Studio's own MCP save and Test are unchanged (CARD-516).
  - A failing tool is **not saved, mounted, granted or added to tool policy**. The API answers **422** with `detail.message` ("Not registered: <tool> failed the <stage> check. <error>") and `detail.check`; the chat tool returns the same message so Developer can fix the tool and register again. If the sandbox itself cannot start, nothing is registered ("The check could not run ... try again").
  - The result is stored as a `check` block on the native tool row and the MCP record, and, when the Developer chat came from Tools Studio **Submit**, as a `tools_studio_tool_check` journey event plus `tool_checks` on the job (the Submit result lists them). `persisted_tool` stays false.
  - The Tools Studio catalog labels native rows **Checked**, **Checked without a sample call: <reason>** or **Not checked** (tools registered before this change are left alone and still load at startup).
  - Folds in **CARD-517**: the MCP client no longer reports a tool call that crashed its server as success with empty output.
  - New module `src/application/tools/tool_check.py`; `detect_path_safety_violation` moved there from `verification_battery.py` (re-exported until CARD-497 deletes the battery). Honest limit: the sandbox strips secrets from the environment but does not block the network or protect files, so the sample call really runs. Contracts: `tests/unit/tools/test_tool_check.py`, `tests/unit/tools/test_native_packaging_check.py`, MCP additions in `test_mcp_engineering_tools.py` / `test_mcp_client_adapter.py`, `test_oc422` / `test_oc423`, Vitest `card_422` / `card_423`, smoke TC-42 ([CARD-511], [CARD-517]).
- **CARD-496 The Agent Training Factory is gone from the app (ADR-0060)**: The dock has no **Factory** button and there is no Factory window. The Lab Training Monitor drawer, the Train Agent popup and hidden toggle/badge, and the Agent Studio **Agent Training Optimization** panel (with its Open Training Factory button) are removed, along with the code behind them (`studios/factory.js`, `forge/lab_monitor.js`, `chat/training.js`, `chat/train_modal.js`: about 1,400 lines of JS and 420 lines of markup). Now:
  - Agent Studio's **Needs Training Backlog** is labelled **Capability gaps** (ids unchanged). Each gap shows **Open in Skill Studio** (opens Skill Studio on that agent), **Ask Developer** (opens a Developer chat pre-filled with the missing tool through `POST /api/tools_studio/authoring/talk`; if that fails, a toast and Tools Studio for the agent) and **Dismiss**.
  - The "Agent created" chat card says "Add skills in Skill Studio, or open it in Agent Studio." with **Open in Skill Studio** and **Open in Agent Studio** buttons (the old **Open in Studio** button did nothing because the click handler looked in the wrong place for the app callbacks).
  - The Teach needs-tool button class is `btn-escalate-developer` (was `btn-escalate-factory`); `data-factory-escalation` stays until CARD-497.
  - Quick Scaffold stays in Agent Studio after creating an agent instead of jumping to the Factory.
  - Agent Studio **Save** no longer sends `allow_autonomous_training` / `max_training_retries`; the API applies its defaults and nothing reads them (CARD-497 removes them).
  - On load, saved desktop layouts and presets drop any `factory` window, and the stale `autoreiv_factory_selected_agent_id` browser setting is cleared. No server data changes.
  - Chat no longer handles the `auto_train_progress` event or the unused `handoff` stream event, and the app makes no calls to `/api/agent_training_factory/jobs` or `/api/capabilities/scaffold`.
  - Skill Studio is unchanged: it keeps its `/api/agent_training_factory/{scaffold/save,skills,capabilities}` calls and its `factory*` element ids. Its helpers moved from `studios/factory/` to `studios/skill_studio/`. No backend route changes (CARD-497/498 retire the backend).
  - `chat.js` is now 975 lines and `chat/render.js` 758, both under the CARD-397 caps, so the two `chat_monolith_decomposition_397` failures listed in CARD-456 now pass.
  - Contracts: `tests/unit/frontend/card_496_retire_factory_ui.test.js`, smoke TC-1/TC-34/TC-36 updated, new smoke TC-39/TC-40/TC-41 desktop+phone. Deleted Factory-only Vitest files: `factory_studio`, `card_411_factory_layout`, `lab_monitor_{controls,fail_reason,artifact_preview}`, `train_agent_handshake` ([CARD-496]).
...
- **CARD-505 AutoReiv's prompt no longer looks edited because of a trailing newline**: Platform prompts are now compared as normalized text (CRLF/CR to LF, outer spaces and newlines trimmed; inner spacing still counts). The shipped AutoReiv prompt ended in a newline that Agent Studio trims on save, so the stored prompt never matched the platform baseline: every start skipped the prompt update (`promoted_partial`), and a Max-Turns-only save locked AutoReiv as "You edited the system prompt". Now:
  ...
  - One-time start-up migration re-saves the old raw-bytes prompt baselines in the normalized form (settings key `platform_prompt_baseline_normalized`, with a per-agent action). The existing lock repair (`migrate_false_content_locks`) now also unlocks agents whose prompt matches the platform version apart from spacing, and saves a backup first (reason `card505_whitespace_unlock`). A lock stays when the prompt really differs, or when a shipped tool was removed from the agent.
  - `platform-packs/autoreiv/pack.json` no longer ends its prompt with a newline; a hygiene test keeps every shipped prompt trimmed.
  - Contracts: `tests/unit/agent_packs/test_card505_prompt_normalize.py`, `tests/integration/test_card505_prompt_restart.py` ([CARD-505]).
- **CARD-502 An adopted Teach skill is used from the next message and survives restart**: Adopt now saves the skill the same way an Agent Studio tick does. Before, it only edited a throwaway copy of the agent, so the skill was not listed on the next message, and on restart platform promotion reset the list to the platform seed. Agent Studio ticks of extra skills on platform agents (for example `build-agent-pack` on Tutor) were lost on restart for the same reason. Now:
  - New `src/application/agent_packs/skill_list.py` (`persist_agent_profile`, `add_skill_to_agent`) is the one save used by `PUT /api/agents/{id}` and `adopt_skill`. The two copies of the override field list in `agents.py` are gone.
  - Skills the operator adds to a platform agent (by Adopt or by ticking in Agent Studio) are recorded in settings key `platform_operator_added_skills`. Restart promotion puts them back on top of the platform seed, as long as a `SKILL.md` for them still exists. It no longer deletes their folders. Adopt never sets `user_modified`, so platform prompt and stock-skill updates keep flowing.
  - With "Keep my agent customizations" off, a restart backs up the list, then drops the added skills and leaves the `SKILL.md` on disk.
  - Adopt returns `active`, `already_adopted` and `resets_on_restart`. An unknown agent gets 404 and no folder is created. A lesson id that matches one of the agent's platform skills gets 409 "autoreiv already has a platform skill called wiki-inbox. Rename the lesson and try again." and nothing changes. Adopting the same id again keeps one entry and replaces its text.
  - The card and toast text comes from that answer (new `chat/adopt_message.js`): "Cite Sources is on for autoreiv from your next message.", "Updated Cite Sources for autoreiv. It is used from your next message.", plus " Keep my agent customizations is off, so it will be removed on the next restart." as a warning when that applies. If the agent did not pick the skill up, you get an error instead. Reloaded cards say "On for autoreiv." The three hard-coded "Skill mounted ... Active for your next message" strings are gone. `render.js` went from 828 to 815 lines.
  - Contracts: `tests/unit/skills/test_card502_adopt_persists.py`, `tests/integration/test_card502_adopt_restart.py`, `tests/unit/frontend/adopt_status_502.test.js`, smoke TC-37 desktop+phone ([CARD-502]).
- **CARD-500 Teach uses your lesson and the reply you picked**: The Teach box now sends `{session_id, message_id, guidance}` (it sent `operator_guidance` and `target_agent_id`, so the typed lesson was dropped). The distiller builds its prompt from the clicked reply and the question right before it, with that turn's tool calls and results only (it used the latest turn out of the last 20 messages). An id that is not a reply in that chat returns 404 "That message is not a reply in this chat. Teach from one of the agent's replies." with no fallback. `/learn <text>` opens Teach for the latest reply (it sent no id and got a 422); with no reply yet it says "Send a message first, then teach from the reply." and does not call Distill. Distill, Adopt and Tools Studio errors read as one sentence via shared `readableError()` in `utils/formatters.js` (FastAPI 422 lists show the field and message, never `[object Object]`). The proposal card shows the real skill name and the plain-English slip and remedy (it showed "Synthesized Skill" and default text). Needs-tool cards show only Ask Developer and Dismiss. Cards reloaded from chat history get the session, agent and toast, so Adopt there behaves like a fresh card. Adopt taking effect is CARD-502; distill timeout is CARD-503. Contracts: `tests/unit/frontend/teach_distill_contract_500.test.js`, `tests/unit/skills/test_card500_distill_turn.py`, `tests/unit/web/test_card500_distill_router.py`, smoke TC-35/TC-36 ([CARD-500]).
