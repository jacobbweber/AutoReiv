...
- CARD-165 Ready (`AutoReiv.Kernel`, `AutoReiv.Orchestration`, `AutoReiv.Web`, `AutoReiv.Agents` - CARD-165):
  - **Autonomous In-Flight Tool Synthesis & Capability Backlog**: Scaffolding specification for per-agent "Allow Autonomous Training" (`allow_autonomous_training`) and "Max Auto-Train Retries" (`max_training_retries`) controls in Agent Studio, JIT in-flight sandbox tool synthesis with automatic turn resumption upon 100% clean verification pass, and SQLite-backed "Needs Training" backlog queue with one-click deliberate training.
