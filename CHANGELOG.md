...
- CARD-165 In Review (`AutoReiv.Kernel`, `AutoReiv.Orchestration`, `AutoReiv.Web`, `AutoReiv.Agents`, `AutoReiv.Frontend` - CARD-165):
  ...
  - **Chat Studio Training Trigger & Modal Alignment**: Added `[⚡ Train in Lab]` message action button (`.train-lab-msg-btn`) to assistant chat bubbles to immediately queue capability gaps, resolved active agent from `state.selectedAgentId` so the popup title dynamically reflects the agent (e.g. "Train Hyper-V (Lab Loop)"), ensured `#trainTargetLocation` remains empty for workspace paths rather than agent IDs, and populated objectives with the user's command rather than bot apology text [REQ-FACT-028].
