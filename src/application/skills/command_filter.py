...
class DangerousCommandFilter:
    ...
    PROHIBITED_PATTERNS: List[Tuple[Pattern[str], str]] = [
        (
            re.compile(r"\brm\s+(-[a-zA-Z]*r[a-zA-Z]*f*|-r|-f)\s+(/|/\*|~|~\*)(\s|$)", re.IGNORECASE),
            "Recursive root/home directory deletion (rm -rf /)",
        ),
        (re.compile(r"\bmkfs(\.[a-zA-Z0-9]+)?\s+", re.IGNORECASE), "Filesystem format operation (mkfs)"),
        (
            re.compile(r"\bdd\s+.*of=/dev/(sd[a-z]|nvme[0-9]n[0-9]|hd[a-z]|disk[0-9])", re.IGNORECASE),
            "Raw block device overwriting (dd)",
        ),
