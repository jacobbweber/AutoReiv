...
- [ ] `[REQ-DATA-012]`: Sibling tab lists user packs and reads/edits `SKILL.md` on disk. Disk is the source of truth.
- [ ] `[REQ-DATA-013]`: Open pack lists tools from that `SKILL.md`.
- [ ] `[REQ-DATA-014]`: Job templates are a stub. Playbook != job template. `jobs.template_id` stays nullable.
- [ ] Automated tests green via `pytest`.
- [ ] Zero lint errors via `ruff check` on touched Python. Frontend checks if the tab ships.
