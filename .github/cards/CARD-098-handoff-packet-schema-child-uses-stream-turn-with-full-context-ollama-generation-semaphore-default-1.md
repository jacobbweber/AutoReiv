...
- [x] `[REQ-ORCH-036]`: Packet requires goal/facts/constraints/done_when/budget. Child gets zero parent transcript. Missing field fails closed.
- [x] `[REQ-ORCH-037]`: Child uses `stream_turn` with full context. No nested `complete()`. No 32k `run_turn` cap on this path. CARD-091 aclose still holds.
- [x] `[REQ-ORCH-038]`: Semaphore default 1. Extra work queues. Batch > cap errors; no silent truncate.
