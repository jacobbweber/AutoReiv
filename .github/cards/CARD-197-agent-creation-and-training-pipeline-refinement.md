...
- [ ] AutoReiv's `build-agent-pack` skill implements Socratic discovery, asking 3–4 high-leverage clarifying questions to produce industry-standard system prompts with role, boundaries, protocol, and output contracts.
- [ ] Quick-scaffold agent creation modal available in Factory Studio and Agent Studio.
- [ ] Post-creation handoff card renders in chat with one-click transition into Factory Studio pre-scoped with the new agent.
- [ ] All 8 Factory phase prompts in `prompt_registry.py` upgraded with rigorous agentic AI standards, schema enforcement, and clear context variables.
- [ ] Self-healing verification retry loop (up to 2 attempts with traceback) in `FactoryOrchestrator` before failing a training job.
- [ ] HITL Promotion modal renders tabbed deliverable inspection (Runbook markdown preview, Python tool code with syntax formatting, and `pack.json` diff).
- [ ] Per-phase execution duration metrics tracked in SQLite and displayed on visual stepper nodes.
- [ ] Tool name collision guard preventing duplicate callable names in target agent packs.
- [ ] Skills and tools generated by the factory strictly adhere to the Matt Pocock progressive disclosure standard and atomic tool return schemas.
- [ ] Automated unit tests for Socratic prompt builder, self-healing loop, duration tracking, and deliverable inspectors.
