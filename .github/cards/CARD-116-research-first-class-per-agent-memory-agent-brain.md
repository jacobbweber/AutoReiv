...
2. **Pack-Scoped**: Lives directly inside the agent's isolated pack storage (`$DATA_DIR/packs/<agent_id>/<agent_slug>_storage.db`), ensuring full lifecycle alignment during backup, export, and deletion.
...
- [ ] `[REQ-MEM-002]`: Isolated SQLite brain schema implemented in `$DATA_DIR/packs/<agent_id>/<agent_slug>_storage.db`.
